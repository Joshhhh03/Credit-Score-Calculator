...
export class CreditDataValidator {
  ...
  static validateHousing(data: any): HousingValidation {
    return {
      ...
      landlordName: this.validateLandlordName(
        data.landlordName,
        data.housingType,
      ),
      monthlyRent: this.validateMonthlyRent(data.monthlyRent, data.housingType),
      leaseStartDate: this.validateLeaseStartDate(
        data.leaseStartDate,
        data.housingType,
      ),
    };
  ...
  static validateBanking(data: any): BankingValidation {
    return {
      ...
      monthlyExpenses: this.validateMonthlyExpenses(
  ...
  static validateDateOfBirth(dateString: string): ValidationResult {
    ...
    if (age < 18) {
      return {
        isValid: false,
        error: "You must be at least 18 years old to apply for credit",
      };
    }
  ...
  static validateAnnualSalary(salary: string | number): ValidationResult {
    ...
    if (salaryNum < 1000) {
      return {
        isValid: false,
        error: "Please enter a valid annual salary (minimum $1,000)",
      };
    }
    ...
    if (salaryNum > 10000000) {
      return {
        isValid: false,
        error: "Please enter a realistic salary amount",
      };
    }
  ...
  static validateEmploymentStartDate(dateString: string): ValidationResult {
    ...
    if (yearsAgo > 50) {
      return {
        isValid: false,
        error: "Please enter a valid employment start date",
      };
    }
  ...
  static validateLandlordName(
    ...
    if (housingType === "rent") {
      if (!landlordName || landlordName.trim().length === 0) {
        return {
          isValid: false,
          error: "Landlord name is required for renters",
        };
      }
      ...
      if (landlordName.trim().length < 2) {
        return {
          isValid: false,
          error: "Landlord name must be at least 2 characters",
        };
      }
    }
  ...
  static validateMonthlyRent(
    ...
    if (housingType === "rent") {
      ...
      if (!rentNum || isNaN(rentNum)) {
        return {
          isValid: false,
          error: "Monthly rent is required for renters",
        };
      }
      ...
      if (rentNum < 100) {
        return {
          isValid: false,
          error: "Please enter a realistic rent amount (minimum $100)",
        };
      }
      ...
      if (rentNum > 50000) {
        return {
          isValid: false,
          error: "Please enter a realistic rent amount",
        };
      }
  ...
  static validateLeaseStartDate(
    ...
    if (housingType === "rent" && !dateString) {
      return {
        isValid: false,
        error: "Lease start date is required for renters",
      };
    }
  ...
  static validateMonthlyIncome(income: string | number): ValidationResult {
    ...
    if (incomeNum > 1000000) {
      return {
        isValid: false,
        error: "Please enter a realistic monthly income",
      };
    }
  ...
  static validateMonthlyExpenses(
    expenses: string | number,
    income: string | number,
  ): ValidationResult {
    const expensesNum =
      typeof expenses === "string" ? parseFloat(expenses) : expenses;
    const incomeNum = typeof income === "string" ? parseFloat(income) : income;
    ...
    if (expensesNum > 1000000) {
      return {
        isValid: false,
        error: "Please enter a realistic monthly expense amount",
      };
    }
  ...
  static validateCompleteProfile(data: any): {
    ...
    const summary = {
      personalInfo: Object.values(personalInfo).every((field: any) =>
        typeof field === "object" && field.isValid !== undefined
          ? field.isValid
          : Object.values(field).every((subField: any) => subField.isValid),
      ),
      employment: Object.values(employment).every(
